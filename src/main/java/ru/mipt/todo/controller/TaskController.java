...
public class TaskController {
    ...
    @GetMapping("/due-soon")
    @Operation(summary = "Задачи с дедлайном в ближайшие 7 дней")
    public ResponseEntity<List<TaskResponseDto>> getTasksDueSoon() {
        List<TaskResponseDto> tasks = taskService.getTasksDueSoon();
        return ResponseEntity.ok()
                .header("X-Total-Count", String.valueOf(tasks.size()))
                .header("X-API-Version", apiVersion)
                .body(tasks);
    ...
    @Operation(summary = "Массовое завершение задач")
    @ApiResponse(responseCode = "200", description = "Задачи завершены")
    public ResponseEntity<List<TaskResponseDto>> bulkComplete(@RequestBody List<Long> ids) {
        List<TaskResponseDto> completed = taskService.bulkCompleteTasks(ids);
        return ResponseEntity.ok()
                .header("X-API-Version", apiVersion)
                .body(completed);
    }
